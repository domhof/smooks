 ...
 * CSV Reader.
 * <p/>
 * This CSV Reader can be plugged into the Smooks (for example) in order to convert a CSV based message stream into a
 * stream of SAX events to be consumed by the DOMBuilder.
 * 
 * <h3>Configuration</h3>
 ...
 * (that it where the tokens present in the line is less than the number of tokens expected) would be garbled and a WARN
 * log statement was provided. Now, you can decide if you want those lines to be parsed too, this is accomplished by
 * setting strict="false" on the config.
 * 
 * <h3>String manipulation functions</h3>
 * String manipulation functions can be defined per field. These functions are executed before that the data is
 * converted into SAX events. The functions are defined after the field name, separated with a question mark. So a field
 * definition with string functions could look like this: firstname?trim,lastname?right_trim,gender?upper_case Take a
 * look in the Smooks manual for a list of all available functions.
 * 
 * <h3>Ignoring Fields</h3>
 * To ignore a field in a CSV record set, just insert the string "<b>$ignore$</b>" for that field in the fields
 * attribute.
 * 
 * <h3>Simple Java Bindings</h3>
 * A simple java binding can be configured on the reader configuration. This allows quick binding configuration where
 * the CSV records map cleanly to the target bean. For more complex bindings, use the Java Binging Framework.
 * 
 * <h3>Example Usage</h3>
 ...
 * </pre>
 * <p/>
 * Within Smooks, the stream of SAX events generated by the "Acme-Order-List" message (and this parser) will generate an
 * event stream equivalent to the following:
 * 
 * <pre>
 * &lt;csv-set&gt;
 * 	&lt;csv-record number="1"&gt;
