 ...
 * CSV Reader.
 * <p/>
 * This CSV Reader can be plugged into the Smooks (for example) in order to
 * convert a CSV based message stream into a stream of SAX events to be consumed
 * by the DOMBuilder.
 * 
 * <h3>Configuration</h3>
 ...
 * Strict parsing was the only option until Smooks 1.2.x, whereby lines that
 * would not comply with the provided tokens (that it where the tokens present
 * in the line is less than the number of tokens expected) would be garbled and
 * a WARN log statement was provided. Now, you can decide if you want those
 * lines to be parsed too, this is accomplished by setting strict="false" on the
 * config.
 * 
 * <h3>String manipulation functions</h3>
 * String manipulation functions can be defined per field. These functions are
 * executed before that the data is converted into SAX events. The functions are
 * defined after the field name, separated with a question mark. So a field
 * definition with string functions could look like this:
 * firstname?trim,lastname?right_trim,gender?upper_case Take a look in the
 * Smooks manual for a list of all available functions.
 * 
 * <h3>Ignoring Fields</h3>
 * To ignore a field in a CSV record set, just insert the string
 * "<b>$ignore$</b>" for that field in the fields attribute.
 * 
 * <h3>Simple Java Bindings</h3>
 * A simple java binding can be configured on the reader configuration. This
 * allows quick binding configuration where the CSV records map cleanly to the
 * target bean. For more complex bindings, use the Java Binging Framework.
 * 
 * <h3>Example Usage</h3>
 * So the following configuration could be used to parse a CSV stream into a
 ...
 * </pre>
 * <p/>
 * Within Smooks, the stream of SAX events generated by the "Acme-Order-List"
 * message (and this parser) will generate an event stream equivalent to the
 * following:
 * 
 * <pre>
 * &lt;csv-set&gt;
 ...
 * </pre>
 * <p/>
 * Other profile based transformations can then be used to transform the CSV
 * records in accordance with the requirements of the consuming entities.
 * 
 * @author tfennelly
 */
public class CSVReader implements SmooksXMLReader, VisitorAppender {
    ...
    private static final String USE_FIELDNAME = "$usename$";

    private enum SpecialFieldType {
        IGNORE_FIELD, USE_FIELDNAME
    }
    ...
    @ConfigParam(name = "head-line", defaultVal = "0")
     ...
     * 
     * @see
     * org.milyn.xml.SmooksXMLReader#setExecutionContext(org.milyn.container
     * .ExecutionContext)
     */
    ...
    public void parse(InputSource csvInputSource) throws IOException, SAXException {
        ...
        try {
            ...
            while ((csvRecord = csvLineReader.readNext()) != null) {
                ...
                // If we reached here it means that this line has to be in the
                // sax stream
                // hence we first add the record number attribute on the
                // csv-record element
                attrs.addAttribute(XMLConstants.NULL_NS_URI, RECORD_NUMBER_ATTR, RECORD_NUMBER_ATTR, "xs:int",
                        Integer.toString(lineNumber));
                // if this line is truncated, we add the truncated attribute
                // onto the csv-record element
                ...
                for (Field field : fields) {
                    ...
                    if (field.ignore()) {
                        int toSkip = parseFieldDirective(SpecialFieldType.IGNORE_FIELD, fieldName);
                        ...
                        if (headLineNames == null) {
                            logger.warn("No headline defined!");
                            break;
                        }
                        ...
                        if (toSkip > 1) {
                            for (int i = 0; i < toSkip; i++) {
                                ...
                                // fieldName = headLineNames[fieldIt];
                                fieldName = headLineNames[recordIt];

                                contentHandler.startElement(XMLConstants.NULL_NS_URI, fieldName, "", EMPTY_ATTRIBS);
                                contentHandler.characters(csvRecord[recordIt].toCharArray(), 0,
                                        csvRecord[recordIt].length());
                    ...
                    // Don't insert the element if the csv record does not
                    // contain it!!
            ...
            // These properties need to be reset for every execution (e.g. when
            // reader is pooled).
            contentHandler = null;
            execContext = null;
        }
    ...
    private int parseFieldDirective(SpecialFieldType myFieldType, String field) {
        String op;
        if (myFieldType == SpecialFieldType.IGNORE_FIELD)
            op = field.substring(IGNORE_FIELD.length());
        else if (myFieldType == SpecialFieldType.USE_FIELDNAME)
            op = field.substring(USE_FIELDNAME.length());
        else
    ...
    private class Field {
        ...
        private final boolean useFieldname;
        ...
        public Field(String name, StringFunctionExecutor stringFunctionExecutor) {
            ...
            useFieldname = name.startsWith(USE_FIELDNAME);
        }
